...
public class ElPlusPlusNormalizer implements Normalizer
{
	public ElPlusPlusNormalizer(OWLOntology ontology, Set<GCI> expressions)
	{
	...
	private void PhaseOne()
	{
		while(!phaseOneExpressions.isEmpty())
		{
			...
			OWLObject subclass = current_gci.getSubClass();
			OWLClassExpression rhs = current_gci.getSuperClass();
			if(current_gci.getSubclassType().equals("individual"))
			{
			...
			{
				OWLClassExpression lhs = (OWLClassExpression)subclass;
				...
				else if(lhs.getClassExpressionType() == ClassExpressionType.OBJECT_INTERSECTION_OF)
				{
	                ...
	                else if(operands.size() >= 2)
	                {
	                	...
	                	phaseOneExpressions.add(new GCI(newIntersection1, rhs, "expression"));
	                	...
	                	{                		
	                		...
	                		phaseOneExpressions.add(new GCI(newIntersection2, newName, "expression"));
	                	}
				...
				else if(lhs.getClassExpressionType() == ClassExpressionType.OBJECT_SOME_VALUES_FROM)
				{
					...
					else if(!Utilities.isInBC(filler))
					{
	                	...
	                	phaseOneExpressions.add(new GCI(newSomeValuesFrom, rhs, "expression"));
	                	phaseOneExpressions.add(new GCI(filler, newName, "expression"));
					}
				}
			}
			}
	...
	private void PhaseTwo()
	{
		while(!phaseTwoExpressions.isEmpty())
		{
			...
			OWLObject lhs = current_gci.getSubClass();
			OWLClassExpression rhs = current_gci.getSuperClass();
			...
			if(!Utilities.isInBC(lhs) && !Utilities.isInBC(rhs))
			{
            	...
            	phaseTwoExpressions.add(new GCI(newName, rhs, current_gci.getSubclassType()));
			}
			else if(rhs.getClassExpressionType() == ClassExpressionType.OBJECT_SOME_VALUES_FROM)
			{
				OWLObjectSomeValuesFrom someValuesFrom = (OWLObjectSomeValuesFrom) rhs;
				...
				else if(!Utilities.isInBC(filler))
				{
	            	...
	            	OWLObjectPropertyExpression property = someValuesFrom.getProperty();
                	OWLObjectSomeValuesFrom newSomeValuesFrom = dataFactory.getOWLObjectSomeValuesFrom(property, newName);
                	phaseTwoExpressions.add(new GCI(lhs, newSomeValuesFrom, current_gci.getSubclassType()));
                	phaseTwoExpressions.add(new GCI(newName, filler, current_gci.getSubclassType()));
				}
			}
			...
			else if(rhs.getClassExpressionType() == ClassExpressionType.OBJECT_INTERSECTION_OF)
			{
                ...
                if(operands.size() > 1)
                {
                	...
                	phaseTwoExpressions.add(new GCI(lhs, newIntersection, current_gci.getSubclassType()));
                }
			...
			else
			{
				...
				{
					OWLClassExpression expressionLhs = (OWLClassExpression)lhs;
					//secondo caso: lhs è una congiunzione, C1 e C2 devono essere in BC
					//e D deve essere in BC o bottom
					if(expressionLhs.getClassExpressionType() == ClassExpressionType.OBJECT_INTERSECTION_OF && Utilities.isInBCOrBottom(rhs))
					{
						OWLObjectIntersectionOf intersection = (OWLObjectIntersectionOf) lhs;
		                ArrayList<OWLClassExpression> operands = new ArrayList<OWLClassExpression>(intersection.getOperands());
					...
					else if(expressionLhs.getClassExpressionType() == ClassExpressionType.OBJECT_SOME_VALUES_FROM && Utilities.isInBCOrBottom(rhs))
					{
						OWLObjectSomeValuesFrom someValuesFrom = (OWLObjectSomeValuesFrom) lhs;
						OWLClassExpression filler = someValuesFrom.getFiller();
